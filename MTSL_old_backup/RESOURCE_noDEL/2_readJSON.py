...
csv_file = open("借券賣出_從"+lines[0]+"至"+lines[len(lines)-1]+".csv", "w")

csv_file.writelines('年月日,股票代號,當日賣出,當日還券,當日調整,當日餘額,次一營業日可限額\n')
for json_file in lines:
	input_file = open ("SOURCE_JSON\\"+json_file+'.json',encoding='utf-8')
