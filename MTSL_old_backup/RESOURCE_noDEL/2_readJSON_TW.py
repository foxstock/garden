...
csv_file = open("TW借券賣出_從"+lines[0]+"至"+lines[len(lines)-1]+".csv", "w")

csv_file.writelines('年月日,股票代號,當日賣出,當日還券,當日調整,當日餘額,次一營業日可限額,mktype\n')
for json_file in lines:
	input_file = open ("SOURCE_JSON_TW\\"+json_file+'.json',encoding='utf-8')
	json_array = json.load(input_file)
	#print(json_array['data'])
	for i in json_array['data']:
		if(i[0]!=''):
			print("%s,%s,%s,%s,%s,%s,%s,0" % (fix_datetime_format(json_file),i[0],num_comma_clear(i[9]),num_comma_clear(i[10]),num_comma_clear(i[11]),num_comma_clear(i[12]),num_comma_clear(i[13])))
