...
def remove_old_tmp_file(today):
    ...
    for filename in os.listdir(folder_path):
        if filename.endswith('.json') and not filename.startswith(today) and not filename.startswith('temp'):
            ...
            # 刪除檔案
            os.remove(file_path)
            print(f'已刪除檔案：{folder_path} {filename}')
...
def download_process(today_str,toHHMMSS_str):
    ...
    url=f"https://www.twse.com.tw/rwd/zh/lending/TWT72U?date={today_str}&selectType=SLBNLB&response=json&_=1693287551157"
    source=my.get_web_content(url,15)
    if(my.utf8len(source)<5000):
        #表示今天數據還沒更新
        print("LENDX 今天數據還沒更新，請稍後重試...")
    else:
        try:
            # create_dir_if_not_exist(SOURCE_JSON_DIR+today_str[:4]+'\\')
            old_txt,new_txt='',''
            with open(SOURCE_JSON_DIR+"temp.json","w") as f:
                f.write(source.replace('\\','\\\\'))
            with open(SOURCE_JSON_DIR+"temp.json","r") as f:
                new_txt=f.read()
            # if os.path.exists(SOURCE_JSON_DIR+today_str+".json"):
            if os.path.exists(f"{SOURCE_JSON_DIR}\\{today_str}.json"):
                with open(f"{SOURCE_JSON_DIR}\\{today_str}.json","r") as f:
                    old_txt=f.read()
            print('TW_OLD:',my.utf8len(old_txt),'TW_NEW:',my.utf8len(new_txt))
            if not compare_last_download(old_txt,new_txt):
                with open(f"{SOURCE_JSON_DIR}\\{today_str}.json","w") as f:
                    f.write(source.replace('\\','\\\\'))
                print("LENDX 今天新數據已下載完成!")
                dataProcessCount+=1
            else:
                print("LENDX 今天無新數據!")
            ### 清理 0KB檔案
    ...
    if dataProcessCount>0: #表示上市或上櫃有更新
        ...
        with open(FINAL_CSV_DIR+today_str+".csv", "w") as csv_file:
            ...
            for i in json_array['data']:
                if(i[0]!='' and i[0]!='合計'):
                    csv_file.writelines("%s,%s,%s,%s\n" % (my.fix_datetime_format(today_str),i[0],my.num_comma_clear(i[5]),('0' if i[8]=='集中市場' else '1')))
...
if __name__ == "__main__":
    try:
        ...
        #將當前視窗名設為"MTSL-DAILY"
        w=WindowMgr()
        w.set_cmd_title("L-E-N-D-X-T-E-M-P")
        while 1:
            w.find_window_wildcard("LENDX-DAILY")
            if w.is_exist==True:
                print("LENDX-DAILY FOUND.KILL IT!")
                w.set_foreground()
        ...
        w.find_window_wildcard("L-E-N-D-X-T-E-M-P")
        w.set_cmd_title("LENDX-DAILY")
        #變數設定
        base_dir = os.getcwd()
        SOURCE_JSON_DIR=base_dir+"\\SOURCE_JSON\\"
        FINAL_CSV_DIR=base_dir+"\\FINAL_CSV\\"
        #先確定相關資料夾存在，若不存在則新增
        create_dir_if_not_exist(SOURCE_JSON_DIR)
