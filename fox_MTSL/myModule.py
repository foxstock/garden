...
說明:常用函式庫整合
"""
import re,requests,random,time,os,pathlib,datetime
...
#取得台灣證交所網站公告的休市日
def get_twse_rest_day(queryYear):
    url="http://www.twse.com.tw/zh/holidaySchedule/holidaySchedule?queryYear="+str(queryYear)+"&submitBtn=%E6%9F%A5%E8%A9%A2"
...
def check_today_is_workday_and_not_twse_rest_day(todayStr):
    #oday_str=strftime("%Y%m%d", time.localtime())
    chineseYear=int(todayStr[0:4])-1911
    print('民國'+str(chineseYear)+'年'+todayStr[4:6]+'月'+todayStr[6:8]+'日')
    twse_rest_arr=get_twse_rest_day(chineseYear)
    if check_work_day(todayStr):
        if todayStr[4:8] in twse_rest_arr:
            return False
        else:
            return True
    else:
...
def check_today_is_workday_from_textfile(dateStr,work_day_str):
